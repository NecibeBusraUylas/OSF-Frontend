...
(function () {
  const bigImages = document.querySelectorAll(
    "section.product div.overview div.large img"
  );
  const smallImages = document.querySelectorAll(
    "section.product div.overview div.small img"
  );

  // Small -> Big

  for (let i = 0; i < smallImages.length; i++) {
    smallImages[i].addEventListener(
      "click",
      () => {
...
if (text.innerHTML.length > 100) {
  ...
  const share = document.querySelector(
    "section.product div.overview div.about div.share"
  );
...
const colorSelect = document.querySelector(
  "section.product div.overview div.about div.colorpick > select"
);

// Color select ..............................
(function () {
  const colors = document.querySelectorAll(
    "section.product div.overview div.about div.colorpick > ul > li"
  );
...
const amount = document.querySelector(
  "section.product div.overview div.about div.add div.amountpick > input"
);

// Amount select ..............................
(function () {
  const minus = document.querySelector(
    "section.product div.overview div.about div.add div.amountpick > button.minus"
  );
  const plus = document.querySelector(
    "section.product div.overview div.about div.add div.amountpick > button.plus"
  );
...
(function () {
  ...
  addThisItem.addEventListener(
    "click",
    () => {
      addToCart(
        amount.value,
        colorSelect.value === "Dark Grey"
          ? "https://firebasestorage.googleapis.com/v0/b/osf-academy-frontend.appspot.com/o/pdp%2FPG.10216885.JJ169XX.PZ-small.jpg?alt=media&token=f36eafcb-bc25-4f37-9f9a-f7fe987d9e9e"
          : "https://firebasestorage.googleapis.com/v0/b/osf-academy-frontend.appspot.com/o/pdp%2FPG.10216885.JJ8UTXX.PZ-small.jpg?alt=media&token=af8a6fcd-dc28-44e1-8116-39d671d2c71b",
        document.querySelector("section.breadcrumbs h2").innerText,
        document
          .querySelector("section.product div.overview div.about span.price")
          .innerText.slice(1)
      );
...
(function () {
  let addToCart = document.querySelectorAll(
    "section.pop-items button.addToCart"
  );
...
(function () {
  let addToWishlist = document.querySelectorAll(
    "section.pop-items button.addToWishlist"
  );
...
function addItem(branchName) {
  ...
  firebase
    ...
    .once("value", (snap) => {
      ...
      firebase.database().ref(`/amounts/${branchName}/${newID}`).update({
        image:
          "https://firebasestorage.googleapis.com/v0/b/osf-academy-frontend.appspot.com/o/items%2Fitem2.jpg?alt=media&token=765a5d78-8358-43dc-baa0-438b4570f127",
...
function makeFutureSlider() {
  ...
  if (window.innerWidth <= 440 && futureDots === null) {
    ...
    const futureDotsBox = document.querySelector(
      "section.pop-items ul#popular.future-dots"
    );

    // Find items
    let futureItems = document.querySelectorAll(
      "section.pop-items div.items div.item"
    );
    ...
    futureDots = document.querySelectorAll(
      "section.pop-items ul#popular.future-dots > li"
    );
...
function makeFutureImages() {
  ...
  if (window.innerWidth <= 440 && futureImages === null) {
    // Find Future slider box
    const futureDotsBox = document.querySelector(
      "section.product ul#images.future-dots"
    );

    // Find items
    const futureItems = document.querySelectorAll(
      "section.product div.overview div.small img"
    );

    const futureItemsBig = document.querySelectorAll(
      "section.product div.overview div.large img"
    );
    ...
    // Get pointers to the dots
    futureImages = document.querySelectorAll(
      "section.product ul#images.future-dots > li"
    );
    ...
    // Add event listeners to the dots
    for (let i = 0; i < futureItems.length; i++) {
      futureImages[i].addEventListener(
        "click",
        () => {
